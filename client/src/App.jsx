...
function App() {
  ...
  const [isDeleting, setIsDeleting] = useState(null); // Track ID of image being deleted
  ...
  const handleDelete = async (imageId) => {
    setIsDeleting(imageId); // Indicate which image is being deleted
    try {
      await axios.delete(`http://localhost:8800/images/${imageId}`);
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-10 px-4">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {gallery.map((img) => (
          <div key={img.id} className="bg-white rounded shadow overflow-hidden relative">
            <img
            ...
            <button
              ...
              className="absolute top-2 right-2 bg-red-500 text-white rounded-full p-2 hover:bg-red-600 transition focus:outline-none"
